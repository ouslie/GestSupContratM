	...
	<?php
		if($_SESSION['user_id']) //logon user case
		{
			...
			if($rparameters['restrict_ip'])
			{
				...
				if($ipcheck==0) {echo '<div class="space-10"></div><div class="alert alert-danger"><button type="button" class="close" data-dismiss="alert"><i class="fa fa-times-circle text-danger"></i></button><strong><i class="fa fa-times-circle text-danger"></i> '.T_('Erreur').'</strong> : '.T_("Vous n'avez pas accès à ce logiciel.").'<br></div>';}
			} else {$ipcheck=1;}
			
			//check SSO
			if($rparameters['ldap_sso']==1 && isset($_SERVER['REMOTE_USER']) && $_GET['action']!='logout' && $ipcheck==1)
			{
