			...
			<div class="p-3">
			   ...
			   <?php
				...
				<div class="form-group row <?php if(($rright['ticket_tech_disp']==0 && $_GET['action']!='new') || ($rright['ticket_new_tech_disp']==0 && $_GET['action']=='new')) {echo 'd-none';} ?>">
					<div class="col-sm-2 col-form-label text-sm-right pr-0">
						...
						<?php 
							if($lock_tech==0) //case lock field 
							{
								...
								{
									echo '<i title="'.T_('Aucun technicien associé à ce ticket').'" class="fa fa-exclamation-triangle text-danger-m2 text-130"></i>&nbsp;';
								}
							}
							echo T_('Technicien').' :'; 
						?>
					...
					<div class="col-sm-5">
						<select class="form-control col-5 d-inline-block" id="technician" name="technician" onchange="loadVal(); submit(); <?php if($rright['ticket_tech_mandatory']) {echo ' CheckMandatory(); ';}?>" <?php if($rright['ticket_tech']==0 || $lock_tech==1) {echo ' disabled="disabled" ';}?> >
							<?php
				...
				<div class="form-group row <?php if(($rright['ticket_cat_disp']==0 && $_GET['action']!='new') || ($rright['ticket_new_cat_disp']==0 && $_GET['action']=='new')) echo 'd-none';?>">
					<div class="col-sm-2 col-form-label text-sm-right pr-0">
						<label class="mb-0"  for="category">
							<?php if(($globalrow['category']==0) && ($_POST['category']==0)) {echo '<i title="'.T_('Aucune catégorie associée').'" class="fa fa-exclamation-triangle text-danger-m2 text-130"></i>&nbsp;';} ?>
							<?php echo T_('Catégorie').' :'; ?>
						</label>
					</div>
					<div class="col-sm-5">
						<select <?php if($mobile) {echo 'style="max-width:105px;"';}else{echo 'style="width:auto;"';}?>  class="form-control d-inline-block mb-1 mb-md-0" title="<?php echo T_('Catégorie'); ?>" id="category" name="category" onchange="loadVal(); submit(); <?php if($rright['ticket_cat_mandatory']) {echo ' CheckMandatory(); ';} ?>" <?php if(($rright['ticket_cat']==0 && $_GET['action']!='new') || ($rright['ticket_new_cat']==0 && $_GET['action']=='new')) echo ' disabled="disabled" ';?>>
						<?php
		...
		function GetUserInfos(user) {
			...
			$.ajax({ 
				...
				success: function(result){
					//console.error('JSON received :', result)
