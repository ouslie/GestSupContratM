...
if (!isset($_POST['start_availability_d'])) {
    $_POST['start_availability_d'] = '';
}

if (!isset($_POST['end_availability_d'])) {
    $_POST['end_availability_d'] = '';
}
...
if (!isset($_POST['u_agency]'])) {
    $_POST['u_agency]'] = '';
}

if (!isset($_POST['sender_service'])) {
    $_POST['sender_service'] = '';
}

if (!isset($_POST['contrats'])) {
    $_POST['contrats'] = '';
}
...
if (!isset($globalrow['technician'])) {
    $globalrow['technician'] = '';
}

if (!isset($globalrow['time'])) {
    $globalrow['time'] = '';
}
...
if (!isset($globalrow['contrats'])) {
    $globalrow['contrats'] = '';
}

if (!isset($globalrow['creator'])) {
    $globalrow['creator'] = '';
}

if (!isset($globalrow['t_group'])) {
    $globalrow['t_group'] = '';
}

if (!isset($globalrow['u_group'])) {
    $globalrow['u_group'] = '';
}

if (!isset($globalrow['category'])) {
    $globalrow['category'] = '';
}

if (!isset($globalrow['subcat'])) {
    $globalrow['subcat'] = '';
}

if (!isset($globalrow['title'])) {
    $globalrow['title'] = '';
}

if (!isset($globalrow['description'])) {
    $globalrow['description'] = '';
}

if (!isset($globalrow['date_create'])) {
    $globalrow['date_create'] = date("Y-m-d") . ' ' . date("H:i:s");
}

if (!isset($globalrow['date_hope'])) {
    $globalrow['date_hope'] = '';
}

if (!isset($globalrow['date_res'])) {
    $globalrow['date_res'] = '';
}

if (!isset($globalrow['time_hope'])) {
    $globalrow['time_hope'] = '5';
}
...
if (!isset($globalrow['priority'])) {
    $globalrow['priority'] = '';
}

if (!isset($globalrow['state'])) {
    $globalrow['state'] = '1';
}

if ($rparameters['user_limit_service'] == 1 && $rright['dashboard_service_only'] != 0) {
    if (!isset($globalrow['type'])) {
        $globalrow['type'] = '0';
    }
...
if (!isset($globalrow['start_availability'])) {
    $globalrow['start_availability'] = date("Y-m-d") . ' ' . date("H:i:s");
}

if (!isset($globalrow['end_availability'])) {
    $globalrow['end_availability'] = date("Y-m-d") . ' ' . date("H:i:s");
}

if (!isset($globalrow['availability_planned'])) {
    $globalrow['availability_planned'] = 0;
}

if (!isset($globalrow['place'])) {
    $globalrow['place'] = '0';
}

if (!isset($globalrow['criticality'])) {
    $globalrow['criticality'] = '0';
}

if (!isset($globalrow['u_service'])) {
    $globalrow['u_service'] = '0';
}

if (!isset($globalrow['asset_id'])) {
    $globalrow['asset_id'] = '';
}

if (!isset($globalrow['u_agency'])) {
    $globalrow['u_agency'] = '0';
}

if (!isset($globalrow['sender_service'])) {
    $globalrow['sender_service'] = '0';
}

//default values for tech and admin and super
if ($_SESSION['profile_id'] == 4 || $_SESSION['profile_id'] == 0 || $_SESSION['profile_id'] == 3) {
    if (!isset($globalrow['technician'])) {
        $globalrow['technician'] = $_SESSION['user_id'];
    }

    if (!isset($globalrow['user'])) {
        $globalrow['user'] = 0;
    }
    ...
    if (!isset($globalrow['user'])) {
        $globalrow['user'] = $_SESSION['user_id'];
    }
...
<div id="row">
	<div class="col-xs-12">
		<div class="widget-box">
			<form class="form-horizontal" name="myform" id="myform" enctype="multipart/form-data" method="post" action=""
			 onsubmit="loadVal();">
...
$query->closeCursor();
if ($alarm) {
    echo ' <i class="icon-bell-alt orange" title="' . T_('Alarme activée le') . ' ' . $alarm['date_start'] . '" /></i>';
}
...
$query->closeCursor();
if ($plan) {
    echo '<a target="_blank" href="./index.php?page=calendar"><i class="icon-calendar green" title="' . T_('Ticket planifié dans le calendrier le') . ' ' . $plan['date_start'] . '" /></i></a>';
}
...
if ($rright['ticket_next'] != 0) {
    if ($previous[0] != '') {
        echo '<a href="./index.php?page=ticket&amp;id=' . $previous[0] . '&amp;state=' . $globalrow['state'] . '&amp;userid=' . $_GET['userid'] . '"><i title="' . T_('Ticket précédent de cet état') . '" class="icon-circle-arrow-left bigger-130"></i>&nbsp;';
    }

    if ($next[0] != '') {
        echo '<a href="./index.php?page=ticket&amp;id=' . $next[0] . '&amp;state=' . $globalrow['state'] . '&amp;userid=' . $_GET['userid'] . ' "><i title="' . T_('Ticket suivant de cet état') . '" class="icon-circle-arrow-right bigger-130"></i></a>';
    }
...
if ($rright['ticket_print'] != 0 && $_GET['action'] != 'new') {
    ...
    echo "&nbsp;";
    echo '<a target="_blank" href="./ticket_print.php?id=' . $_GET['id'] . '&user_id=' . $_SESSION['user_id'] . '&token=' . $token . '"><i title="' . T_('Imprimer ce ticket') . '" class="icon-print purple bigger-130"></i></a>';
}
...
?>
				...
				<div class="widget-body">
					<div class="widget-main">
						<!-- START sender part -->
						<div class="form-group <?php if (($rright['ticket_user_disp'] == 0 && $_GET['action'] != 'new') || ($rright['ticket_new_user_disp'] == 0 && $_GET['action'] == 'new')) {
    echo 'hide';
}
?>">
							<label class="col-sm-2 control-label no-padding-right" for="user">
								<?php if (($_POST['user'] == 0) && ($globalrow['user'] == 0) && ($u_group == '')) {
    echo '<i title="' . T_('Sélectionner un demandeur') . '" class="icon-warning-sign red bigger-130"></i>&nbsp;';
}
?>
							...
							<div class="col-sm-9">
								...
								<!-- <select autofocus id="user" name="user" onchange="loadVal(); submit();" <?php if (($rright['ticket_user'] == 0 && $_GET['action'] != 'new') || ($rright['ticket_new_user'] == 0 && $_GET['action'] == 'new')) {
    echo ' disabled="disabled" ';
}
?> > -->
								<select autofocus <?php if ($mobile==0) {echo 'class="chosen-select"' ;} else {echo
								 ' style="max-width: 225px;" ' ;}?> id="user" name="user" onchange="loadVal(); submit();"
									<?php if (($rright['ticket_user'] == 0 && $_GET['action'] != 'new') || ($rright['ticket_new_user'] == 0 && $_GET['action'] == 'new')) {
    echo ' disabled="disabled" ';
}
?>
									>
...
?>
								...
								<?php if (($rright['ticket_user'] == 0 && $_GET['action'] != 'new') || ($rright['ticket_new_user'] == 0 && $_GET['action'] == 'new')) {
    echo ' <input type="hidden" name="user" value=' . $globalrow['user'] . ' /> ';
}
...
if ($u_group == '') {
    ...
    if ($row['phone'] != "") {
        echo '<a href="tel:' . $row['phone'] . '"><i title="' . T_('Téléphoner au') . ' ' . $row['phone'] . '" class="icon-phone-sign blue bigger-130"></i></a> <b>';
    }
    if ($mobile == 0) {echo $row['phone'];}
    echo '</b>';
    if ($row['mobile'] != "") {
        echo '&nbsp;&nbsp;&nbsp;<a href="tel:' . $row['mobile'] . '"><i title="' . T_('Téléphoner au') . ' ' . $row['mobile'] . '" class="icon-mobile-phone blue bigger-150"></i></a> <b>';
    }
    if ($mobile == 0) {echo $row['mobile'];}
    echo '</b>';
    if ($row['mail'] != "") {
        echo '&nbsp;&nbsp;&nbsp;<a href="mailto:' . $row['mail'] . '"><i title="' . T_("Envoyer un mail à l'adresse") . ' ' . $row['mail'] . '" class="icon-envelope blue bigger-130"></i></a>';
    }

    if ($row['function'] != "") {
        echo '&nbsp;&nbsp;&nbsp;<i title="' . T_('Fonction') . '" class="icon-user blue bigger-130"></i> ';
    }
    if ($mobile == 0) {echo $row['function'];}
    if ($row['company'] != 0) {
...
if ($mobile == 0) {
    ...
    if ($umodif != '') //case for new ticket without sender
    {
        ...
        $rnn = $rn[0];
        if ($rnn != 0) {
            echo '&nbsp;&nbsp; <i title="' . T_('Autres tickets de cet utilisateur') . '" class="icon-ticket blue bigger-130"></i> ';
        }
...
if ($rright['ticket_service_disp'] != 0) {
    ...
    echo '
				                <div class="form-group ';if ($rright['ticket_new_service_disp'] == 0 && $_GET['action'] == 'new') {echo 'hide';}
    echo ' ' . $service_mandatory . '" >
        							<label class="col-sm-2 control-label no-padding-right" for="u_service">
        							    ';
    if (($_POST['u_service'] == 0) && ($globalrow['u_service'] == 0)) {if ($rright['ticket_service_mandatory'] != 0) {echo '<i title="' . T_('La saisie du service est obligatoire') . '"';} else {echo '<i title="' . T_('Sélectionner un service') . '"';}
        echo 'class="icon-warning-sign red bigger-130"></i>&nbsp;';}
    echo '
        							    ' . T_('Service') . ':
        							</label>
        							<div class="col-sm-8">
        							    <select id="u_service" name="u_service" ';if (($rright['ticket_service'] == 0 && $_GET['action'] != 'new') || ($rright['ticket_new_service'] == 0 && $_GET['action'] == 'new')) {echo 'disabled="disabled"';}
    echo ' onchange="loadVal(); submit();">
											';
    if ($_POST['u_service']) {
    ...
    echo '
										...
										';
    //send data in disabled case
    if ($rright['ticket_service'] == 0 && $_POST['u_service'] == 0 && $globalrow['u_service'] != 0) {
...
if ($rparameters['ticket_type'] == 1) {
    echo '
				                <div class="form-group ';if (($rright['ticket_type_disp'] == 0 && $_GET['action'] != 'new') || ($rright['ticket_new_type_disp'] == 0 && $_GET['action'] == 'new')) {echo 'hide';}
    echo '">
    ...
    echo '
        							...
        							<div class="col-sm-8">
        							    <select  id="type" name="type"';if (($rright['ticket_type'] == 0 && $_GET['action'] != 'new') || ($rright['ticket_new_type'] == 0 && $_GET['action'] == 'new')) {echo 'disabled="disabled"';}
    echo '>';
    ...
    if ($rparameters['user_limit_service'] == 1 && $rright['ticket_type_service_limit'] != 0) {
        ...
        while ($row2 = $query2->fetch()) {
            //select entry
            $selected = '';
            if ($_POST['type'] && $row2['id'] == $_POST['type']) {$selected = 'selected';} elseif ($globalrow['type'] && $row2['id'] == $globalrow['type']) {$selected = 'selected';}
    ...
    echo '
										...
										';
    //send data in disabled case
    if ($rright['ticket_type'] == 0 && $_GET['action'] != 'new') {
        echo '<input type="hidden" name="type" value="' . $globalrow['type'] . '" />';
    }
...
?>
							...
							<div class="col-sm-8 ">
								<select id="technician" name="technician" onchange="loadVal(); submit();" <?php if ($rright['ticket_tech']==0
								 || $lock_tech==1) { echo ' disabled="disabled" ' ; } ?> >
									<?php
...
if ($lock_tech == 1) {
    echo '<input type="hidden" name="technician" value="' . $tech_selected . '" />';
}
...
if ($rparameters['asset'] == 1) {
    if ($rright['ticket_new_asset_disp'] != 0) {
        ...
        echo '
									<div class="form-group ';if (($rright['ticket_new_asset_disp'] == 0 && $_GET['action'] == 'new') || ($rright['ticket_asset_disp'] == 0 && $_GET['action'] != 'new')) {echo 'hide';}
        echo ' ' . $asset_mandatory . '" >
										<label class="col-sm-2 control-label no-padding-right" for="asset">
											';
        if (($_POST['asset_id'] == 0) && ($globalrow['asset_id'] == 0)) {if ($rright['ticket_asset_mandatory'] != 0) {echo '<i title="' . T_("La saisie de l'équipement est obligatoire") . '"';} else {echo '<i title="' . T_('Sélectionner un équipement') . '"';}
            echo 'class="icon-warning-sign red bigger-130"></i>&nbsp;';}
        echo '
											' . T_('Équipement') . ':
										</label>
										<div class="col-sm-8">
											<select id="asset_id" name="asset_id" ';if (($rright['ticket_asset'] == 0 && $_GET['action'] != 'new') || ($rright['ticket_new_asset_disp'] == 0 && $_GET['action'] == 'new')) {echo 'disabled="disabled"';}
        echo ' onchange="loadVal(); submit();">
												';
        if ($_POST['asset_id']) {
        ...
        echo '
											...
											';
        //send data in disabled case
        if ($rright['ticket_asset'] == 0 && $_GET['action'] != 'new') {
            echo '<input type="hidden" name="asset_id" value="' . $globalrow['asset_id'] . '" />';
        }
...
?>
						<!-- END asset part -->
						<!-- START category part -->
						<div class="form-group <?php if (($rright['ticket_cat_disp'] == 0 && $_GET['action'] != 'new') || ($rright['ticket_new_cat_disp'] == 0 && $_GET['action'] == 'new')) {
    echo 'hide';
}
?>">
							<label class="col-sm-2 control-label no-padding-right" for="category">
								<?php if (($globalrow['category'] == 0) && ($_POST['category'] == 0)) {
    echo '<i title="' . T_('Aucune catégorie associée') . '." class="icon-warning-sign red bigger-130"></i>&nbsp;';
}
?>
								<?php echo T_('Catégorie') . ' :'; ?>
							</label>
							<div class="col-sm-8">
								<select title="<?php echo T_('Catégorie'); ?>" id="category" name="category" onchange="loadVal(); submit();"
								 <?php if (($rright['ticket_cat']==0 && $_GET['action'] !='new' ) || ($rright['ticket_new_cat']==0 &&
...
?>
								...
								<?php if (($rright['ticket_cat'] == 0 && $_GET['action'] != 'new') || ($rright['ticket_new_cat'] == 0 && $_GET['action'] == 'new')) {
    echo '<input type="hidden" name="category" value="' . $globalrow['category'] . '" />';
}
//send data in disabled case ?>
								<select title="<?php echo T_('Sous-catégorie'); ?>" id="subcat" name="subcat" onchange="loadVal(); submit();"
								 <?php if (($rright['ticket_cat']==0 && $_GET['action'] !='new' ) || ($rright['ticket_new_cat']==0 &&
								 $_GET['action']=='new' )) { echo ' disabled="disabled" ' ; } ?> >
									<?php
if ($_POST['category']) {$query = $db->query("SELECT * FROM `tsubcat` WHERE cat LIKE '$_POST[category]' ORDER BY name ASC");} else { $query = $db->query("SELECT * FROM `tsubcat` WHERE cat LIKE '$globalrow[category]' ORDER BY name ASC");}
...
$query->closeCursor();
if ($globalrow['subcat'] == 0 && $_POST['subcat'] == 0) {
...
?>
								...
								<?php if (($rright['ticket_cat'] == 0 && $_GET['action'] != 'new') || ($rright['ticket_new_cat'] == 0 && $_GET['action'] == 'new')) {
    echo '<input type="hidden" name="subcat" value="' . $globalrow['subcat'] . '" />';
}
//send data in disabled case?>
								<?php
...
if ($rparameters['user_agency'] == 1) {
    ...
    } else //else display field to select agency
    {
        ...
        echo '
				                <div class="form-group ' . $agency_mandatory . '">
        							<label class="col-sm-2 control-label no-padding-right" for="u_agency">
										';
        if (($_POST['u_agency'] == 0) && ($globalrow['u_agency'] == 0) && ($_SESSION["profile_id"] == 1 || $_SESSION["profile_id"] == 2)) {if ($rright['ticket_agency_mandatory'] != 0) {echo '<i title="' . T_("La saisie de l'agence est obligatoire") . '"';} else {echo '<i title="' . T_('Sélectionner une agence') . '"';}
            echo 'class="icon-warning-sign red bigger-130"></i>&nbsp;';}
        echo '
        							    ' . T_('Agence') . ':
        							</label>
        ...
        while ($row3 = $query3->fetch()) {
            ...
            if ($count == 1 && $_GET['action'] == 'new') {$selected = 'selected';
                $find_agency_id = 1;}
            /*
...
if ($rparameters['ticket_places'] == 1) {
    echo '
							<div class="form-group">
								...
								<div class="col-sm-8">
									<select class="textfield" id="ticket_places" name="ticket_places" ';if ($rright['ticket_place'] == 0 && $_GET['action'] != 'new') {echo 'disabled="disabled"';}
    echo ' >
										';
    if ($_POST['ticket_places']) {
        $query = $db->query("SELECT * FROM `tplaces` ORDER BY name ASC");
        while ($row = $query->fetch()) {
            if ($_POST['ticket_places'] == $row['id']) {
    ...
    echo '
									...
									';
    if ($rright['ticket_place'] == 0 && $_GET['action'] != 'new') {
        echo '<input type="hidden" name="ticket_places" value="' . $globalrow['place'] . '" />';
    }
    //send data in disabled case
    echo '
								</div>
...
?>
							...
							<div class="col-sm-8">
								<input name="title" id="title" type="text" size="<?php if ($mobile == 0) {echo '50';} else {echo '30';}?>"
								 maxlength="100" value="<?php if ($_POST['title']) {echo htmlspecialchars($_POST['title']);} else {echo htmlspecialchars($globalrow['title']);}?>"
								 <?php if ($rright['ticket_title']==0 && $_GET['action'] !='new' ) { echo 'readonly="readonly"' ; } ?> />
							</div>
						</div>
						<!-- END title part -->
...
if ($rright['ticket_description_disp'] != 0) {
    if ($rright['ticket_description_mandatory'] != 0) {
        if ((($_POST['text'] == '' || ctype_space($_POST['text']) == 1 || ctype_space(strip_tags($_POST['text'])) == 1)) && ($globalrow['description'] == '' || (ctype_space(strip_tags($globalrow['description'])) == 1 || strip_tags($globalrow['description']) == ''))) {$ticket_description_mandatory = 'has-error';} else { $ticket_description_mandatory = 'has-success';}
    } else { $ticket_description_mandatory = '';}
} else { $ticket_description_mandatory = '';}
?>
						<div class="form-group <?php if ($rright['ticket_description_disp'] == 0) {echo 'hide';}
echo $ticket_description_mandatory;?>">
...
?>
							...
							<div class="col-sm-8">
								<table border="1" width="<?php if ($mobile == 0) {echo '764';} else {echo '285';}?>" style="border: 1px solid #D8D8D8;"
								 <?php if ($rright['ticket_description']==0) { echo 'cellpadding="10"' ; } ?> >
									<tr>
										<td>
											<?php
if ($rright['ticket_description'] != 0 || $_GET['action'] == 'new') {
    ...
    if ($_GET['action'] == 'new' && !$_POST['user']) {echo '';}
    echo '
												</div>
...
?>
						...
						<div class="form-group <?php if (($rright['ticket_resolution_disp'] == 0 && $_GET['action'] != 'new') || ($rright['ticket_new_resolution_disp'] == 0 && $_GET['action'] == 'new')) {
    echo 'hide';
}
?>">
							<label class="col-sm-2 control-label no-padding-right" for="resolution">
								<?php echo T_('Résolution'); ?> :</label>
							<div class="col-sm-8">
...
?>
						<div class="form-group  <?php if ($rright['ticket_date_create_disp'] == 0) {
    echo 'hide';
}
?>">
							<label class="col-sm-2 control-label no-padding-right" for="date_create">
								<?php echo T_('Date de la demande'); ?> :</label>
							<div class="col-sm-8">
								<input type="hidden" name="hide" id="hide" value="1" />
								<input autocomplete="off" type="text" name="date_create" id="date_create" value="<?php if ($_POST['date_create']) {
    echo $_POST['date_create'];
} else {
    echo $globalrow['date_create'];
}
?>"
								 <?php if ($rright['ticket_date_create']==0) { echo 'readonly="readonly"' ; } ?> >
							</div>
						</div>
...
?>
						...
						<div class="form-group <?php echo $date_hope_error;if ($rright['ticket_date_hope_disp'] == 0) {
    echo ' hide ';
}
?>">
							...
							<div class="col-sm-8">
								<input autocomplete="off" type="text" id="date_hope" name="date_hope" onchange="loadVal();" value="<?php if ($_POST['date_hope']) {
    echo $_POST['date_hope'];
} else {
    echo $globalrow['date_hope'];
}
?>"
								 <?php if ($rright['ticket_date_hope']==0) { echo 'readonly="readonly"' ; } ?>>
								<?php
...
$query->closeCursor();
if ($resultdiff[0] > 0 && ($globalrow['state'] != "3" && $globalrow['state'] != "4")) {
    echo "<i title=\"Date de résolution dépassée de $resultdiff[0] jours\" class=\"icon-warning-sign orange bigger-130\" ></i>";
}
...
?>
						<div class="form-group <?php if ($rright['ticket_date_res_disp'] == 0) {
    echo 'hide';
}
?>">
							<label class="col-sm-2 control-label no-padding-right" for="">
								<?php echo T_('Date de résolution'); ?> :</label>
							<div class="col-sm-8">
								<input autocomplete="off" type="text" id="date_res" name="date_res" value="<?php if ($_POST['date_res']) {
    echo $_POST['date_res'];
} else {
    echo $globalrow['date_res'];
}
?>"
								 <?php if ($rright['ticket_date_res']==0) { echo 'readonly="readonly"' ; } ?>>
							</div>
						</div>
						<!-- END resolution date part -->
						<!-- START time part -->
						<div class="form-group <?php if ($rright['ticket_time_disp'] == 0) {
    echo 'hide';
}
?>">
							<label class="col-sm-2 control-label no-padding-right" for="time">
								<?php echo T_('Temps passé'); ?> :</label>
							<div class="col-sm-8">
...
?>
						...
						<div class="form-group <?php if ($rright['ticket_time_hope_disp'] == 0) {
    echo 'hide';
}
?>">
							<label class="col-sm-2 control-label no-padding-right" for="time_hope">
								<?php if (($globalrow['time_hope'] < $globalrow['time']) && $globalrow['state'] != '3') {
    echo "<i class=\"icon-warning-sign red bigger-130\" title=\"Le temps est sous-estimé.\"></i>";
}
...
if ($_GET['action'] == 'new') {?>
								...
								<select autofocus <?php if ($mobile==0) {echo 'class="chosen-select"' ;} else {echo
								 ' style="max-width: 225px;" ' ;}?> id="contrats" name="contrats" onchange="loadVal(); submit();"
									<?php if (($rright['ticket_user'] == 0 && $_GET['action'] != 'new') || ($rright['ticket_new_user'] == 0 && $_GET['action'] == 'new')) {
    echo ' disabled="disabled" ';
}
    ?>
									...
									<?php
$query = $db->query("SELECT tcontrats.id, tcontrats.service, tcontratstype.nom AS nametypecontrat, month(date_fin) as month FROM tcontrats INNER JOIN tcontratstype ON  tcontrats.type = tcontratstype.id WHERE tcontrats.user ='$_POST[user]' AND tcontrats.status='1' ORDER BY tcontrats.id ASC");
while ($row = $query->fetch()) {
	if ($_POST['contrats'] != '') {if ($_POST['contrats'] == $row['id']) {
		echo '<option value="' . $row['id'] . '" selected>' . $row['id'] . ' - ' . $row['nametypecontrat'] . ' - ' . $row['service'] . ' - ' . $row['month'] . '</option>';
	} else {
		echo '<option value="' . $row['id'] . '">' . $row['id'] . ' - ' .$row['nametypecontrat'] . ' - ' . $row['service'] . ' - ' . $row['month'] . '</option>';
	}
	} else {if ($globalrow['contrats'] == $row['id']) {
		echo '<option value="' . $row['id'] . '" selected>' . $row['id'] . ' - ' . $row['nametypecontrat'] . '' . $row['service'] . ' - ' . $row['month'] . '</option>';
	} else {
		echo '<option value="' . $row['id'] . '">' . $row['id'] . ' - ' . $row['nametypecontrat'] . ' - ' . $row['service'] . ' - ' . $row['month'] . '</option>';
	}
    ...
    ?>
									...
									<?php
$query = $db->query("SELECT tcontrats.id, tcontrats.service, tcontratstype.nom AS nametypecontrat, month(date_fin) as month FROM tcontrats INNER JOIN tcontratstype ON  tcontrats.type = tcontratstype.id WHERE tcontrats.user ='$globalrow[user]' AND tcontrats.status='1' ORDER BY tcontrats.id ASC");
    while ($row = $query->fetch()) {
        if ($_POST['contrats'] != '') {if ($_POST['contrats'] == $row['id']) {
            echo '<option value="' . $row['id'] . '" selected>' . $row['id'] . ' - ' . $row['nametypecontrat'] . ' - ' . $row['service'] . ' - ' . $row['month'] . '</option>';
        } else {
...
$query = $db->query("SELECT * FROM `tcontrats` WHERE id ='$globalrow[contrats]'");
    ...
    ?>
						...
						<div class="form-group <?php echo $priority_error;if ($rright['ticket_priority_disp'] == 0) {
    echo 'hide';
}
?>">
...
if ($rparameters['user_limit_service'] == 1 && $rright['dashboard_service_only'] != 0) {
    ...
    while ($row2 = $query2->fetch()) {
        //select entry
        $selected = '';
        if ($_POST['priority'] && $row2['id'] == $_POST['priority']) {$selected = 'selected';} elseif ($globalrow['priority'] && $row2['id'] == $globalrow['priority']) {$selected = 'selected';}
...
?>
						...
						<div class="form-group <?php echo $criticality_error;if ($rright['ticket_criticality_disp'] == 0) {
    echo 'hide';
}
?>">
							...
							<div class="col-sm-8">
								<select id="criticality" name="criticality" <?php if ($rparameters['availability']==1) {echo
								 'onchange="loadVal(); submit();"' ;}if ($rright['ticket_criticality']==0) {echo 'disabled' ;}?>>
									<?php
...
if ($rparameters['user_limit_service'] == 1 && $rright['dashboard_service_only'] != 0) {
    ...
    while ($row2 = $query2->fetch()) {
        //select entry
        $selected = '';
        if ($_POST['criticality'] && $row2['id'] == $_POST['criticality']) {$selected = 'selected';} elseif ($globalrow['criticality'] && $row2['id'] == $globalrow['criticality']) {$selected = 'selected';}
...
?>
						...
						<!-- START state part -->
						<div class="form-group <?php if ($rright['ticket_state_disp'] == 0) {
    echo 'hide';
}
?>">
							<label class="col-sm-2 control-label no-padding-right" for="state">
								<?php echo T_('État'); ?> :</label>
							<div class="col-sm-8">
...
if ($rparameters['availability'] == 1) {
    if (
        ...
        echo '
        						   	<div class="form-group ';if ($rright['ticket_availability_disp'] == 0) {
            echo 'hide';
        }
        ...
        echo '">
        						    	...
        						    	<div class="col-sm-8">
            						    	<input  type="text" id="start_availability_d" name="start_availability_d"  value="' . $start_availability_d . '"';
        echo '"';
        if ($rright['ticket_availability'] == 0) {
        ...
        echo '">
        					    		...
        					    		<div class="col-sm-8">
        					    			<input type="checkbox"';if ($globalrow['availability_planned'] == 1) {echo "checked";}
        echo ' name="availability_planned" value="1" />
        					    		</div>
        					    	</div>
        					    	';
    }
