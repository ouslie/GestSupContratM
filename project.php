...
if($rright['project'])
{
	...
	if($_GET['action']=='add')
	{
		if($_POST['save'])
		{
			...
			$_POST['user_id']=strip_tags($_POST['user_id']);

			$qry=$db->prepare("INSERT INTO `tprojects` (`name`, `user_id`) VALUES (:name, :user_id)");
			$qry->execute(array('name' => $_POST['name'], 'user_id' => $_POST['user_id']));
		...
		<fieldset>
			<div class="col-xs-12">
				<form method="POST" enctype="multipart/form-data" name="myform" id="myform" action="" >
					...
					<br />
					<label for="name">'.T_('UserID').' :</label>
					<input type="text" style="width:auto;" class="form-control form-control-sm d-inline-block" name="user_id" maxlength="50" value="'; echo $_POST['user_id']; echo '">
					<br />
					<br />
	...
	} elseif($_GET['action']=='edit') //////////////////////////////////////////////////////////// edit project
	{
		...
		if($rright['contrat_displayuser']) {
			$qry=$db->prepare("SELECT COUNT(id) FROM `tprojects` WHERE `disable`=0  AND `user_id` = :user_id");
		...
		//for each project
		if($rright['contrat_displayuser']) {
			$qry=$db->prepare("SELECT `id`,`name` FROM `tprojects` WHERE `disable`=0 AND `user_id` = :user_id ORDER BY `id` DESC");
			$qry->execute(array('user_id' => $_SESSION['user_id']));
		...
		while($project=$qry->fetch()) 
		{
			...
			{	
				if($row2['state'] == 3) {
					$time_resolved = $time_resolved + $row2['time'];
				}
				if($row2['state']!=3) {
					$finish=0;
					$time_inprogress = $time_inprogress + $row2['time'];
			...
			<h4 class="text-blue-m1 mb-3 border-b-1 brc-grey-l1 pb-1 pt-2">
				<i class="fa fa-flag '.$flag.'"></i> '.T_("Projet").' n°'.$project['id'].' : '.$project['name'].' | Ticket résolu time  : '.decimal_to_time($time_resolved).'   | Ticket en cours time  : '.decimal_to_time($time_inprogress).' | Ticket Total time  : '.decimal_to_time($time_inprogress + $time_resolved).' ';
				if(!$rright['contrat_displayuser']) {
				echo '
			...
			<div id="smartwizard-1" class="sw-main sw-theme-circles">
				<ul class="mx-auto nav nav-tabs step-anchor" style="width:95% !important">
					';
					...
					{
						//get ticket data
						$qry3=$db->prepare("SELECT `id`,`title`,`state`,`date_hope`,`date_res`,`time`  FROM `tincidents` WHERE id=:id");
						...
						<li data-target="" class="nav-item '.$class.'" >
							...
							<span class="step-description">
								<a title="'.T_('Ouvrir le ticket associé à cette tâche').'" target="_blank " href="index.php?page=ticket&id='.$ticket['id'].'">
									<i class="fa fa-ticket-alt text-'.$color.'"></i> '.$ticket['id'].' : '.truncate($ticket['title'],20).'
								</a>
								<div class="p-0"></div>
								<i title="'.$date_label.'" class="fa fa-calendar text-'.$color.'"></i> '.$date.'<br/>
								<i title="'.$date_label.'" class="fa fa-calendar text-'.$color.'"></i> '.decimal_to_time($ticket['time']).'
							</span>
