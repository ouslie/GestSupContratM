					...
					<span class="card-toolbar">
						<?php 
							...
							if(($ip_asset || $wifi_asset || $iface) && $rparameters['asset_ip'])
							{
								...
								if($wol_mac) {echo '<a style="width:31px; height:29px; padding:5px 0px 0px 0px;" class="btn btn-xs btn-warning" href="./index.php?page=asset&id='.$globalrow['id'].'&action=wol&mac='.$wol_mac.'&'.$url_get_parameters.'"><i title="'.T_("Allumer cet équipement avec l'adresse MAC:").' '.$wol_mac.' " class="fa fa-power-off text-130"></i></a>&nbsp;&nbsp;';}
							}
