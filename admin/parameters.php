...
if($_POST['submit_connector'] || $_POST['test_ldap'])
{
	...
	if($_POST['mail_password'] && !preg_match('/gs_en/',$_POST['mail_password']))
	{
		...
		//update tparameters
		$qry=$db->prepare("UPDATE `tparameters` SET `mail_password`=:mail_password WHERE `id`=:id");
		$qry->execute(array(
			'mail_password' => $enc_mail_password,
			'id' => 1
			));
	}
	if($_POST['ldap_password'] && !preg_match('/gs_en/',$_POST['ldap_password']))
	{
		...
		//update tparameters
		$qry=$db->prepare("UPDATE `tparameters` SET `ldap_password`=:ldap_password WHERE `id`=:id");
		$qry->execute(array(
			'ldap_password' => $enc_ldap_password,
			'id' => 1
			));
	}
	...
	if($_POST['imap_password'] && !preg_match('/gs_en/',$_POST['imap_password']))
	{
		...
		//update tparameters
		$qry=$db->prepare("UPDATE `tparameters` SET `imap_password`=:imap_password WHERE `id`=:id");
		$qry->execute(array(
			'imap_password' => $enc_imap_password,
			'id' => 1
			));
	}
