...
while ($row = $qry->fetch()) {
	...
	$find_old = $find_old[1];
	if ($find_old != '12:00:00') $date_thread = date_convert($row['date']);
	else $date_thread = '';
...
if ($rparameters['mail_from_adr'] == '') {
	...
	$emetteur = $rparameters['mail_from_adr'];
}
...
if ($rparameters['imap'] == 1 && $rparameters['imap_reply'] == 1) {
	$msg = '---- ' . T_('Repondre au dessus de cette ligne') . ' ----';
} else {
	$msg = '';
}
$msg .= '
					...
					<tr bgcolor="' . $rparameters['mail_color_bg'] . '" >
					  <td>
						<font color="' . $rparameters['mail_color_text'] . '">
							';
if ($rparameters['mail_txt']) {
	$msg .= T_($rparameters['mail_txt']);
}
$msg .= '
						</font>
						<br />
...
//add tag in mail to split fonction of imap connector
if ($rparameters['imap'] == 1 && $rparameters['imap_reply'] == 1) {
	$msg .= '---- ' . T_('Repondre au dessus du ticket') . ' ----';
} else {
	$msg .= '';
}
...
if ($send == 1) {
		...
		foreach ($matches[0] as $img) {
			if (strpos($img, 'base64') !== false) {
				if ($rparameters['debug']) {
					echo 'DEBUG: Images base64 detected conversion (' . $img . ')<br />';
				}
				//generate cid
				$id = 'img' . ($i++);
				...
				if ($rparameters['debug']) {
					echo 'DEBUG: Images no base64 detected (' . $img . ')<br />';
				}
			}
		}
	} 
	...
	if ($rparameters['mail_from_adr'] != '') {
		$mail->AddReplyTo($rparameters['mail_from_adr']);
	} elseif ($techrow['mail'] != '') {
	...
	if ($_POST['usercopy6'] != '') {
		if (substr($_POST['usercopy6'], 0, 1) == 'G') {
			...
			$mail->AddCC("$_POST[usercopy6]");
			$dest_mail = 1;
		}
	}
	if ($_POST['manual_address'] != '') {
		$mail->AddCC("$_POST[manual_address]");
		$dest_mail = 1;
	}
