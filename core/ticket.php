...
if (($_GET['action'] == "delete") && ($rright['ticket_delete'] != 0)) {
	//disable ticket
	$qry = $db->prepare("UPDATE `tincidents` SET `disable`=:disable WHERE `id`=:id");
	$qry->execute(array(
		'disable' => 1,
		'id' => $_GET['id']
	));
	//disable associate events
	$qry = $db->prepare("UPDATE `tevents` SET `disable`=:disable WHERE `incident`=:incident");
	$qry->execute(array(
		'disable' => 1,
		'incident' => $_GET['id']
	));
...
if ($_POST['modify'] || $_POST['quit'] || $_POST['mail'] || $_POST['upload'] || $save == "1" || $_POST['send'] || $_POST['action']) {
	...
	if (($rright['ticket_date_hope_mandatory'] != 0) && ($_POST['date_hope'] == '') && ($_POST['technician'] == $_SESSION['user_id'])) {
		$error = T_('Merci de renseigner la date de résolution estimé');
	}
	if (($rright['ticket_priority_mandatory'] != 0) && ($_POST['priority'] == '')) {
		$error = T_('Merci de renseigner la priorité');
	}
	if (($rright['ticket_criticality_mandatory'] != 0) && ($_POST['criticality'] == '')) {
		$error = T_('Merci de renseigner la criticité');
	}
	if (($rright['ticket_description_mandatory'] != 0) && (ctype_space($_POST['text']) || $_POST['text'] == '' || ctype_space(strip_tags($_POST['text'])) == 1) || strip_tags($_POST['text']) == '') {
		$error = T_('Merci de renseigner la description de ce ticket');
	}
	if (($rright['ticket_asset_mandatory'] != 0) && ($rparameters['asset'] == 1) && ($_POST['asset_id'] == 0)) {
		$error = T_("Merci de renseigner l'équipement");
	}
	if (($rright['ticket_title_mandatory'] != 0) && ($_POST['title'] == '')) {
		$error = T_('Merci de renseigner le titre de ce ticket');
	}
	if (($rright['ticket_agency_mandatory'] != 0) && ($rparameters['user_agency'] == 1) && ($_POST['u_agency'] == 0)) {
		...
		if ($row2[0] > 1 && $_SESSION['profile_id'] != 4) {
			$error = T_("Merci de renseigner l'agence");
		}
	}
	if (($rright['ticket_tech_mandatory'] != 0) && ($_POST['technician'] == '0')) {
		$error = T_('Merci de renseigner le technicien associé à ce ticket');
	}
	if (($rright['ticket_service_mandatory'] != 0) && ($_POST['u_service'] == 0)) {
		$error = T_('Merci de renseigner le service');
	}
	...
	if ($rparameters['user_limit_ticket'] == 1 && $ruser['limit_ticket_number'] != 0 && $ruser['limit_ticket_days'] != 0 && $ruser['limit_ticket_date_start'] != '0000-00-00' && ($_SESSION['profile_id'] == 1 || $_SESSION['profile_id'] == 2)) {
		...
		if ($nbticketremaining <= 0) {
			$error = T_('Votre limite de ticket est atteinte, prenez contact avec votre administrateur pour créditer votre compte') . '.';
		}
	}
	...
	if ($rparameters['company_limit_ticket'] == 1 && ($_SESSION['profile_id'] == 1 || $_SESSION['profile_id'] == 2)) {
		...
		if ($rcompany['limit_ticket_number'] != 0 && $rcompany['limit_ticket_days'] != 0 && $rcompany['limit_ticket_date_start'] != '0000-00-00') {
			...
			if ($nbticketremaining <= 0) {
				$error = T_('La limite de ticket attribuée pour votre société est atteinte, prenez contact avec votre administrateur pour créditer votre compte.');
			}
		}
	}
	...
	if ($error == '0') {
		...
		if ($globalrow['technician'] == 0 && $_POST['technician'] != 0 && $globalrow['state'] == '5' && $_POST['state'] == $globalrow['state'] && $t_group == '') {
			if ($rparameters['debug'] == 1) {
				echo "<b>AUTO CHANGE STATE:</b> from 5 to 1 reason technician change detected (globalrow[state]=$globalrow[state] POST[state]=$_POST[state])<br />";
			}
			$_POST['state'] = '1';
		...
		if ($globalrow['t_group'] == 0 && $globalrow['state'] == '5' && $_POST['state'] == $globalrow['state'] && $t_group != '') {
			if ($rparameters['debug'] == 1) {
				echo "<b>AUTO CHANGE STATE:</b> from 5 to 1 reason technician group change detected (globalrow[state]=$globalrow[state] POST[state]=$_POST[state] t_group=$t_group)<br />";
			}
			$_POST['state'] = '1';
			$qry = $db->prepare("INSERT INTO `tthreads` (`ticket`,`date`,`author`,`type`,`state`) VALUES (:ticket,:date,:author,:type,:state)");
			$qry->execute(array('ticket' => $_GET['id'], 'date' => $datetime, 'author' => $_SESSION['user_id'], 'type' => 5, 'state' => $_POST['state']));
		...
		if ((($_POST['resolution'] != '') && ($_POST['resolution'] != '\'\'')) && ($globalrow['technician'] == $_SESSION['user_id']) && ($_POST['state'] == '1')) {
			if ($rparameters['debug'] == 1) {
				echo "<b>AUTO CHANGE STATE</b> from 5 to 2 reason technician add resolution thread detected<br />";
			}
			$_POST['state'] = '2';
			$qry = $db->prepare("INSERT INTO `tthreads` (`ticket`,`date`,`author`,`type`,`state`) VALUES (:ticket,:date,:author,:type,:state)");
			$qry->execute(array('ticket' => $_GET['id'], 'date' => $datetime, 'author' => $_SESSION['user_id'], 'type' => 5, 'state' => $_POST['state']));
		...
		if (($_POST['resolution'] != '') && ($_POST['resolution'] != '\'\'') && ($_POST['technician'] == $_SESSION['user_id']) && ($_POST['state'] == '1') && ($_GET['action'] == 'new')) {
			if ($rparameters['debug'] == 1) {
				echo "<b>AUTO CHANGE STATE</b> from 5 to 2 reason technician add resolution thread on new ticket detected<br />";
			}
			$_POST['state'] = '2';
			$qry = $db->prepare("INSERT INTO `tthreads` (`ticket`,`date`,`author`,`type`,`state`) VALUES (:ticket,:date,:author,:type,:state)");
			$qry->execute(array('ticket' => $_GET['id'], 'date' => $datetime, 'author' => $_SESSION['user_id'], 'type' => 5, 'state' => $_POST['state']));
		...
		if ($_POST['technician'] == '' && $t_group == '') {
			if ($rparameters['debug'] == 1) {
				echo "<b>AUTO CHANGE STATE</b> to default state $rparameters[ticket_default_state] reason no technician or technician group associated with ticket<br />";
			}
	...
	if ($_POST['user']) {
		...
		if ($row['tech'] != '') {
			if ($rparameters['debug'] == 1) {
				echo '<br /><b>AUTO TECH CHANGE:</b> Auto assignement of this ticket, because technician attachment is detected.<br />';
			}
	...
	if ($_POST['user'] && !$_POST['u_service'] && $rright['ticket_service_disp'] == 0) {
		...
		if ($_POST['state'] != 3) {
			$u_service = $row[0];
		} elseif ($_POST['state'] == 3 && $_GET['action'] == 'new') {
			$u_service = $row[0];
		} else {
			$u_service = $globalrow['u_service'];
		}
		if ($rparameters['debug'] == 1) {
			echo ' post_u_service=' . $u_service . '<br />';
		}
	...
	if (($_GET['action'] == 'new') && ($error == "0")) {
		...
		if ($_POST['technician'] == $_SESSION['user_id']) {
			$techread = 1;
			$techread_date = date("Y-m-d H:i:s");
		} //read ticket  
		
		//insert ticket
		$qry = $db->prepare("INSERT INTO `tincidents` 
		(`user`,`type`,`u_group`,`u_service`,`u_agency`,`sender_service`,`technician`,`t_group`,`title`,`description`,`date_create`,`date_hope`,`date_res`,`priority`,`criticality`,`state`,`creator`,`time`,`time_hope`,`category`,`subcat`,`techread`,`techread_date`,`place`,`asset_id`,`start_availability`,`end_availability`,`availability_planned`,`contrats`)
		VALUES 
		(:user,:type,:u_group,:u_service,:u_agency,:sender_service,:technician,:t_group,:title,:description,:date_create,:date_hope,:date_res,:priority,:criticality,:state,:creator,:time,:time_hope,:category,:subcat,:techread,:techread_date,:place,:asset_id,:start_availability,:end_availability,:availability_planned,:contrats)");
		$qry->execute(array(
			'user' => $_POST['user'],
		...
		if ($globalrow['technician'] == '') {
			$techread = 1;
			$techread_date = date("Y-m-d H:i:s");
		} //read ticket case when it's an unassigned ticket.
...
if ($_POST['close'] && $rright['ticket_close'] != 0) {
	...
	//auto send mail
	$_POST['state'] = 3;
	if ($rparameters['mail_auto'] == 1 && $_POST['upload'] == '') {
		include('./core/auto_mail.php');
	}
...
if ($globalrow['time_hope'] != 0 && ($rright['ticket_time_disp'] != 0 && $rright['ticket_time_hope_disp'] != 0)) {
	...
	if (($globalrow['time'] != '1') && ($globalrow['time_hope'] != '1') && ($globalrow['time_hope'] >= $globalrow['time'])) {
		$percentage = ' <span title="' . T_("Pourcentage d'avancement du ticket basé sur le temps passé et estimé") . '">(' . $percentage . '%)</span> ';
	} else {
		$percentage = '';
	}
}
