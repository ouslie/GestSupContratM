...
if($rparameters['ldap'])
{
	...
	if(($_GET['action']=='simul') || ($_GET['action']=='run') || ($_GET['ldaptest']==1) || ($_GET['ldap']==1) || ($ldap_auth==1))
	{
		...
		if($ldapbind) {
			...
			$ldap_connection='<i title="'.T_('Le connecteur ne fonctionne pas vérifier vos paramètres').'" class="fa fa-times-circle text-danger text-130"></i> '.T_('Le connecteur ne fonctionne pas vérifier vos paramètres').'';
			//log
			if($rparameters['log'])
			{
